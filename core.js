...
Blue.Vector3D = function(x,y,z){
	this.x = x || 0;
	this.y = y || 0;
	this.z = z || 0;
};

Blue.Vector3D.prototype = {
	...
	proj : function(v){ return new Blue.Vector3D(v.x - this.x, v.y - this.y, v.z - this.z) }, // TODO rename!!!
	...
	normalize : function(){
	    var n = this.norm();
		return new Blue.Vector3D( this.x / n, this.y / n, this.z / n )
	},

	// subtract a vector 'v' from the vector 
	sub : function(v){
		return new Blue.Vector3D(this.x - v.x, this.y - v.y, this.z - v.z);
	},
	...
	multiply : function(s){
		return new Blue.Vector3D(this.x * s, this.y * s, this.z * s);
	},
	...
	clone : function(){
		return new Blue.Vector3D( this.x, this.y, this.z )
	}
};

Blue.Object3D = function( position ){
	this.position = position;
};

Blue.Object3D.prototype = {
